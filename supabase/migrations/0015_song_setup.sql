alter table public.items
  add column if not exists tuning text not null default 'standard',
  add column if not exists capo smallint;

alter table public.items drop constraint if exists items_capo_range;
alter table public.items
  add constraint items_capo_range check (capo is null or capo between 1 and 12);
