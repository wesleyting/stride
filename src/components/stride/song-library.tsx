  ...
  return <>
    ...
    <div className={`mt-6 grid gap-4 ${pending ? "cursor-progress" : ""}`} onDragOver={(event) => { if (event.target !== event.currentTarget || dragged?.type !== "folder" || !dropTarget?.startsWith("folder:")) return; event.preventDefault(); }} onDrop={(event) => { if (event.target !== event.currentTarget || dragged?.type !== "folder" || !dropTarget?.startsWith("folder:")) return; event.preventDefault(); const targetId = dropTarget.slice("folder:".length); moveFolder(dragged.id, targetId, folderDropAfter); finishDrag(); }}>
      {groups.map((group) => {
        ...
        const rowProps = (song: ItemRecord) => ({ song, folders, referenceUrls: referencesBySong[song.id], dragged: dragged?.type === "song" && dragged.id === song.id, dropPosition: songDrop?.id === song.id ? songDrop.position : null, onDragStart: (event: React.DragEvent<HTMLElement>) => { draggedRef.current = true; event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", song.id); createVisibleDragImage(event); setDragged({ type: "song" as const, id: song.id }); }, onDragEnd: finishDrag, onDragOver: (event: React.DragEvent<HTMLElement>) => { if (dragged?.type !== "song" || dragged.id === song.id) return; event.preventDefault(); event.stopPropagation(); const bounds = event.currentTarget.getBoundingClientRect(); setDropTarget(null); setSongDrop({ id: song.id, position: event.clientY < bounds.top + bounds.height / 2 ? "before" : "after" }); }, onDrop: (event: React.DragEvent<HTMLElement>) => { event.preventDefault(); event.stopPropagation(); if (dragged?.type === "song") { const peers = layoutSongs.filter((peer) => peer.id !== dragged.id && (peer.folder_id ?? null) === group.folderId && peer.is_hidden === song.is_hidden).sort((a, b) => a.sort_order - b.sort_order); const targetIndex = peers.findIndex((peer) => peer.id === song.id); const bounds = event.currentTarget.getBoundingClientRect(); const position = event.clientY < bounds.top + bounds.height / 2 ? "before" : "after"; const beforeId = position === "before" ? song.id : peers[targetIndex + 1]?.id; moveSong(dragged.id, group.folderId, beforeId); } finishDrag(); }, onOpen: () => { if (!draggedRef.current) { beginRouteNavigation(); router.push(`/songs/${song.slug}`); } } });
