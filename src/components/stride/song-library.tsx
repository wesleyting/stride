...
export function SongLibrary({ songs, entries }: { songs: ItemRecord[]; entries: EntryRecord[] }) {
  ...
  return <>
    <div className="mt-6 grid gap-2 lg:grid-cols-[minmax(14rem,1fr)_12rem_10rem_auto]"><label className="relative block"><span className="sr-only">Search songs</span><Search className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-stone-400" aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search songs…" className="h-10 w-full rounded-lg border border-stone-300 bg-white pr-3 pl-9 text-sm shadow-sm transition hover:border-stone-400 focus:border-stone-500 focus:ring-2 focus:ring-stone-500/20" /></label><label><span className="sr-only">Sort songs</span><select value={sort} onChange={(event) => setSort(event.target.value)} className="h-10 w-full cursor-pointer rounded-lg border border-stone-300 bg-white px-3 text-sm shadow-sm transition hover:border-stone-400 focus:ring-2 focus:ring-stone-500/20"><option value="recent">Recently practiced</option><option value="name">Name</option><option value="pinned">Pinned first</option><option value="difficulty-high">Hardest first</option><option value="difficulty-low">Easiest first</option></select></label><label><span className="sr-only">Filter by difficulty</span><select value={difficulty} onChange={(event) => setDifficulty(event.target.value)} className="h-10 w-full cursor-pointer rounded-lg border border-stone-300 bg-white px-3 text-sm shadow-sm transition hover:border-stone-400 focus:ring-2 focus:ring-stone-500/20"><option value="all">Any difficulty</option>{[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value} star{value === 1 ? "" : "s"}</option>)}</select></label><button type="button" aria-pressed={pinnedOnly} onClick={() => setPinnedOnly((current) => !current)} className={`h-10 cursor-pointer rounded-lg border px-3 text-sm font-medium transition focus-visible:ring-2 focus-visible:ring-stone-500 ${pinnedOnly ? "border-stone-900 bg-stone-900 text-white" : "border-stone-300 bg-white text-stone-700 hover:bg-stone-50"}`}>Pinned only</button></div>
    <div className="mt-4 grid gap-3">
      {filtered.length ? filtered.map((song) => {
        ...
        return <article key={song.id} className="group grid min-h-18 gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 transition hover:border-stone-300 hover:bg-stone-50 hover:shadow-sm focus-within:bg-stone-50 sm:grid-cols-[minmax(0,1fr)_10rem_7rem_10rem] sm:items-center">
          <Link href={`/songs/${song.slug}`} className="grid min-w-0 gap-1.5 rounded-md focus-visible:ring-2 focus-visible:ring-stone-500"><h2 className="truncate text-sm font-semibold text-stone-950 group-hover:underline">{titleCaseSongName(song.name)}</h2><SongSetup tuning={song.tuning} capo={song.capo} compact /></Link>
