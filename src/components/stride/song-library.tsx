...
import { referenceSourceLabel, titleCaseSongName, type ItemRecord, type SongFolderRecord } from "@/lib/stride";
...
export function SongLibrary({ songs, folders, foldersReady, referencesBySong = {}, addedSongSlug, isGuest = false, defaultSongPublic = false, defaultTuning = "standard" }: { songs: ItemRecord[]; folders: SongFolderRecord[]; foldersReady: boolean; referencesBySong?: Record<string, string[]>; addedSongSlug?: string; isGuest?: boolean; defaultSongPublic?: boolean; defaultTuning?: string }) {
  ...
  const roots = layoutFolders.filter((folder) => !folder.parent_id || !knownFolderIds.has(folder.parent_id)).sort((a, b) => a.sort_order - b.sort_order || a.name.localeCompare(b.name));
  const orderedFolders = roots.flatMap((root) => [root, ...layoutFolders.filter((folder) => folder.parent_id === root.id).sort((a, b) => a.sort_order - b.sort_order || a.name.localeCompare(b.name))]);
  const folderGroups = orderedFolders.map((folder) => ({ id: folder.id, folderId: folder.id as string | null, parentFolderId: folder.parent_id, depth: folder.parent_id ? 1 : 0, name: folder.name, hasSubfolders: layoutFolders.some((candidate) => candidate.parent_id === folder.id), songs: filtered.filter((song) => song.folder_id === folder.id) }));
  const groups = [
    ...folderGroups,
    { id: "uncategorized", folderId: null, parentFolderId: null, depth: 0, name: "Uncategorized", hasSubfolders: false, songs: filtered.filter((song) => !song.folder_id || !knownFolderIds.has(song.folder_id)) },
  ].filter((group) => group.songs.length > 0 || (!hasFilters && group.id !== "uncategorized") || (group.hasSubfolders && folderGroups.some((candidate) => candidate.parentFolderId === group.id && candidate.songs.length > 0)) || (dragged?.type === "song" && group.id === "uncategorized"));
  ...
  function moveFolder(folderId: string, targetId: string, after: boolean) {
    ...
    const source = layoutFolders.find((folder) => folder.id === folderId);
    const target = layoutFolders.find((folder) => folder.id === targetId);
    if (!source || !target || (source.parent_id ?? null) !== (target.parent_id ?? null)) return;
    const siblings = layoutFolders.filter((folder) => (folder.parent_id ?? null) === (source.parent_id ?? null) && folder.id !== source.id).sort((a, b) => a.sort_order - b.sort_order);
    const insertionIndex = siblings.findIndex((folder) => folder.id === target.id) + (after ? 1 : 0);
    siblings.splice(insertionIndex, 0, source);
    const order = new Map(siblings.map((folder, index) => [folder.id, index]));
    const next = layoutFolders.map((folder) => order.has(folder.id) ? { ...folder, sort_order: order.get(folder.id)! } : folder);
    setLayoutFolders(next);
    startTransition(async () => {
      const result = await setSongFolderOrderAction(siblings.map((folder) => folder.id));
  ...
  return <>
    ...
    {!foldersReady ? <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">Run migration <code>0026_song_subfolders.sql</code> to enable folders and subfolders.</p> : null}
    <div className={`mt-6 grid gap-4 ${pending ? "cursor-progress" : ""}`} onDragOver={(event) => { if (event.target !== event.currentTarget || dragged?.type !== "folder" || !dropTarget?.startsWith("folder:")) return; event.preventDefault(); }} onDrop={(event) => { if (event.target !== event.currentTarget || dragged?.type !== "folder" || !dropTarget?.startsWith("folder:")) return; event.preventDefault(); const targetId = dropTarget.slice("folder:".length); moveFolder(dragged.id, targetId, folderDropAfter); finishDrag(); }}>
      {groups.map((group) => {
        if (group.parentFolderId && !hasFilters && collapsedFolders.has(group.parentFolderId)) return null;
        ...
        const childSongs = group.hasSubfolders ? folderGroups.filter((candidate) => candidate.parentFolderId === group.id).flatMap((candidate) => candidate.songs) : [];
        const visibleCount = [...group.songs, ...childSongs].filter((song) => !song.is_hidden).length;
        const folderIsDragged = dragged?.type === "folder" && dragged.id === group.id;
        const folderIsTarget = dropTarget === `folder:${group.id}`;
        const rowProps = (song: ItemRecord) => ({ song, folders, referenceUrls: referencesBySong[song.id], dragged: dragged?.type === "song" && dragged.id === song.id, dropPosition: songDrop?.id === song.id ? songDrop.position : null, onDragStart: (event: React.DragEvent<HTMLElement>) => { draggedRef.current = true; event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", song.id); createVisibleDragImage(event); setDragged({ type: "song" as const, id: song.id }); }, onDragEnd: finishDrag, onDragOver: (event: React.DragEvent<HTMLElement>) => { if (dragged?.type !== "song" || dragged.id === song.id) return; event.preventDefault(); event.stopPropagation(); const bounds = event.currentTarget.getBoundingClientRect(); setDropTarget(null); setSongDrop({ id: song.id, position: event.clientY < bounds.top + bounds.height / 2 ? "before" : "after" }); }, onDrop: (event: React.DragEvent<HTMLElement>) => { event.preventDefault(); event.stopPropagation(); if (dragged?.type === "song") { const peers = layoutSongs.filter((peer) => peer.id !== dragged.id && (peer.folder_id ?? null) === group.folderId && peer.is_hidden === song.is_hidden).sort((a, b) => a.sort_order - b.sort_order); const targetIndex = peers.findIndex((peer) => peer.id === song.id); const bounds = event.currentTarget.getBoundingClientRect(); const position = event.clientY < bounds.top + bounds.height / 2 ? "before" : "after"; const beforeId = position === "before" ? song.id : peers[targetIndex + 1]?.id; moveSong(dragged.id, group.folderId, beforeId); } finishDrag(); }, onOpen: () => { if (!draggedRef.current) { beginRouteNavigation(`/songs/${song.slug}`); router.push(`/songs/${song.slug}`); } } });
        return <section key={group.id} aria-labelledby={`folder-${group.id}`} className={`rounded-xl transition-[margin,opacity,background-color] duration-200 ${group.depth ? "ml-4 border-l border-stone-200 pl-3 sm:ml-7" : ""} ${folderIsDragged ? "bg-stone-100 opacity-45" : ""} ${folderIsTarget && dragged?.type === "folder" ? folderDropAfter ? "mb-12" : "mt-12" : ""}`} onDragOver={(event) => { if (!dragged) return; if (dragged.type === "folder") { const source = layoutFolders.find((folder) => folder.id === dragged.id); const target = group.folderId ? layoutFolders.find((folder) => folder.id === group.folderId) : null; if (!source || !target || dragged.id === group.folderId || (source.parent_id ?? null) !== (target.parent_id ?? null)) { setDropTarget(null); return; } } event.preventDefault(); const overHeader = Boolean((event.target as HTMLElement).closest("h2")); if (dragged.type === "song" && songDrop && !overHeader) return; setSongDrop(null); setDropTarget(`folder:${group.id}`); if (dragged.type === "folder") { const header = event.currentTarget.querySelector("h2"); if (header) { const bounds = header.getBoundingClientRect(); setFolderDropAfter(event.clientY > bounds.top + bounds.height / 2); } } }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); const overHeader = Boolean((event.target as HTMLElement).closest("h2")); if (!overHeader && dropSongAtPreview(group.folderId)) return; handleFolderDrop(group.folderId); }}>
          <div className="mb-2 flex items-center gap-1"><h2 id={`folder-${group.id}`} className="min-w-0 flex-1"><button type="button" draggable={Boolean(group.folderId)} onDragStart={(event) => { if (!group.folderId) return; draggedRef.current = true; event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", group.folderId); createVisibleDragImage(event); setDragged({ type: "folder", id: group.folderId }); }} onDragEnd={finishDrag} onClick={() => { if (!draggedRef.current) toggleStoredId(collapsedFoldersStorageKey, collapsedFolders, group.id); }} aria-expanded={expanded} aria-controls={`folder-songs-${group.id}`} className={`flex w-full min-w-0 cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-left transition hover:bg-stone-100 focus-visible:ring-2 focus-visible:ring-stone-500 active:cursor-grabbing ${group.depth ? "text-stone-700" : ""} ${folderIsTarget && dragged?.type === "song" ? "bg-stone-100" : ""}`}><Folder className={`${group.depth ? "size-3.5" : "size-4"} text-stone-400`} aria-hidden="true" /><span className={`min-w-0 flex-1 truncate font-semibold text-stone-900 ${group.depth ? "text-[0.8125rem]" : "text-sm"}`}>{group.name}</span><span className="text-xs tabular-nums text-stone-400">{group.hasSubfolders ? visibleCount : activeSongs.length}</span><ChevronDown className={`size-4 text-stone-400 transition-transform ${expanded ? "rotate-180" : ""}`} aria-hidden="true" /></button></h2><CreateItemModal activitySlug="guitar" activityKind="practice" isGuest={isGuest} folders={foldersReady ? folders : undefined} defaultSongPublic={defaultSongPublic} defaultTuning={defaultTuning} initialFolderId={group.folderId} compact folderName={group.name} /></div>
          <div id={`folder-songs-${group.id}`} hidden={!expanded} className="grid gap-2">{activeSongs.length ? activeSongs.map((song) => <SongRow key={song.id} {...rowProps(song)} justAdded={addedSongSlug === song.slug} highlighted={highlightedSong === song.slug} />) : !hiddenSongs.length && !group.hasSubfolders ? <p className="rounded-xl border border-dashed border-stone-200 px-4 py-5 text-center text-sm text-stone-400">No songs in this folder.</p> : null}{hiddenSongs.length && !hiddenShown ? <HiddenSongsToggle count={hiddenSongs.length} shown={false} onClick={() => toggleStoredId(shownHiddenStorageKey, shownHidden, group.id)} /> : null}{hiddenSongs.length && hiddenShown ? <>{hiddenSongs.map((song) => <SongRow key={song.id} {...rowProps(song)} />)}{!hasFilters ? <HiddenSongsToggle count={hiddenSongs.length} shown onClick={() => toggleStoredId(shownHiddenStorageKey, shownHidden, group.id)} /> : null}</> : null}</div>
        </section>;
      })}
    ...
    {foldersReady ? <div className="mt-3 border-t border-stone-100 px-1 pt-3"><SongFolderManager folders={folders} ready addOnly /></div> : null}
  </>;
}
...
function SongRow({ song, folders, referenceUrls, justAdded = false, highlighted = false, dragged, dropPosition, onDragStart, onDragEnd, onDragOver, onDrop, onOpen }: SongRowProps) {
  ...
  return <article data-song-slug={song.slug} draggable onDragStart={(event) => { if ((event.target as HTMLElement).closest("button, [data-song-controls]")) { event.preventDefault(); return; } onDragStart(event); }} onDragEnd={onDragEnd} onDragOver={onDragOver} onDrop={onDrop} onClick={onOpen} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(); } }} role="link" tabIndex={0} aria-label={`Open ${name}`} className={`group grid min-h-18 cursor-pointer gap-3 rounded-xl border px-4 py-3 transition-[margin,background-color,border-color,box-shadow] duration-200 hover:border-stone-300 hover:shadow-sm focus-visible:ring-2 focus-visible:ring-stone-500 sm:grid-cols-[minmax(0,1fr)_10rem_auto] sm:items-center ${song.is_hidden ? "border-stone-200 bg-stone-50/80 hover:bg-stone-100/80" : "border-stone-200 bg-white hover:bg-stone-50"} ${highlighted ? "border-stone-500 ring-2 ring-stone-300" : ""} ${dragged ? "border-stone-300 bg-stone-100 shadow-none [&>*]:invisible" : ""} ${dropPosition === "before" ? "mt-20" : dropPosition === "after" ? "mb-20" : ""}`}>
    <h3 className={`flex min-w-0 items-center gap-2 truncate text-sm font-semibold ${song.is_hidden ? "text-stone-700" : "text-stone-950"}`}><span className="truncate">{name}</span><ReferenceShortcuts urls={referenceUrls} />{justAdded ? <span className="shrink-0 rounded-full bg-stone-900 px-2 py-0.5 text-[0.625rem] font-semibold uppercase tracking-wide text-white">Just Added</span> : null}</h3>
...
function ReferenceShortcuts({ urls = [] }: { urls?: string[] }) {
  const references = [...new Set(urls.filter(Boolean))].slice(0, 2);
  ...
  return <span data-song-controls role="group" className="flex shrink-0 items-center gap-0.5" aria-label="Song references">{references.map((url) => {
    ...
    return <a key={url} href={url} target="_blank" rel="noreferrer" draggable={false} title={`Open ${label}`} aria-label={`Open ${label} reference`} onClick={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()} className="inline-flex size-6 cursor-pointer items-center justify-center rounded-md text-stone-400 transition-colors hover:bg-stone-100 hover:text-stone-950 focus-visible:ring-2 focus-visible:ring-stone-500"><Icon className="size-3.5" fill={youtube ? "currentColor" : "none"} aria-hidden="true" /></a>;
  })}</span>;
}
