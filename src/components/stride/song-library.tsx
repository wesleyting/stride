...
export function SongLibrary({ songs, folders, foldersReady, referencesBySong = {}, addedSongSlug, isGuest = false, defaultSongPublic = false, defaultTuning = "standard" }: { songs: ItemRecord[]; folders: SongFolderRecord[]; foldersReady: boolean; referencesBySong?: Record<string, string[]>; addedSongSlug?: string; isGuest?: boolean; defaultSongPublic?: boolean; defaultTuning?: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    const folderNames = new Map(layoutFolders.map((folder) => [folder.id, folder.name]));
    return layoutSongs
      .filter((song) => {
        if (!terms.length) return true;
        const urls = referencesBySong[song.id] ?? (song.youtube_url ? [song.youtube_url] : []);
        const searchable = normalize([
          song.name,
          folderNames.get(song.folder_id ?? "") ?? "Uncategorized",
          ...urls.flatMap((url) => [referenceSourceLabel(url), url]),
        ].join(" "));
  ...
  }, [difficulty, layoutSongs, layoutFolders, referencesBySong, pinnedOnly, query]);
