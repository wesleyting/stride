...
export function SongVisibilityButton({ itemId, itemName, hidden }: { itemId: string; itemName: string; hidden: boolean }) {
  ...
  return <button type="button" disabled={pending} onClick={() => startTransition(async () => { const result = await setSongHiddenAction(itemId, !hidden); if (result.success) showToast(hidden ? "Song returned to your repertoire." : "Song hidden from your repertoire."); else showToast(result.error ?? "Could not update the song.", { tone: "error" }); })} title={label} aria-label={label} className={buttonVariants({ variant: "ghost", size: "icon-sm" })}>{hidden ? <Eye aria-hidden="true" /> : <EyeOff aria-hidden="true" />}</button>;
}
