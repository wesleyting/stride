...
export function EditPracticeEntryModal({ entryId, itemSlug, note, rating: initialRating, practicePart, suggestions }: { entryId: string; itemSlug: string; note: string; rating: number | null; practicePart: string; suggestions: string[] }) {
  ...
  return <><button type="button" onClick={() => setOpen(true)} title="Edit practice log" aria-label="Edit practice log" className="inline-flex size-5 shrink-0 cursor-pointer items-center justify-center rounded text-stone-400 opacity-100 transition hover:bg-stone-100 hover:text-stone-800 focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-stone-500 sm:opacity-0 sm:group-hover/log:opacity-100 sm:group-focus-within/log:opacity-100"><Pencil className="size-3" aria-hidden="true" /></button><DialogShell open={open} onOpenChange={setOpen} title="Edit Practice Log" description="Add anything you missed or correct the session details." size="md">{open ? <EditPracticeForm entryId={entryId} itemSlug={itemSlug} note={note} initialRating={initialRating} practicePart={practicePart} suggestions={suggestions} close={() => setOpen(false)} /> : null}</DialogShell></>;
}
...
function EditPracticeForm({ entryId, itemSlug, note, initialRating, practicePart, suggestions, close }: { entryId: string; itemSlug: string; note: string; initialRating: number | null; practicePart: string; suggestions: string[]; close: () => void }) {
  ...
  useEffect(() => { if (state.success) { close(); showToast("Practice log updated."); } }, [close, showToast, state.success]);
