...
import { ChevronDown, ChevronRight, FileImage, NotebookPen } from "lucide-react";
...
import { formatTrackedTime, titleCaseSongName } from "@/lib/stride";

export const dynamic = "force-dynamic";

type PublicSong = { username: string; display_name: string; bio: string; share_practice_logs: boolean; share_song_resources: boolean; song_name: string; song_slug: string; difficulty: number | null; tracked_seconds: number; last_practiced: string | null; youtube_url: string; tuning?: string | null; capo?: number | null };
...
export default async function PublicSongPage({
  ...
  const mediaResult = song.share_song_resources
    ...
    : { data: [], error: null };
  const referencesResult = song.share_song_resources ? await supabase.rpc("get_public_song_references", { profile_username: song.username, public_song_slug: song.song_slug }) : { data: [], error: null };
  const referenceUrls = referencesResult.error ? (song.youtube_url ? [song.youtube_url] : []) : (referencesResult.data ?? []).map((reference: { url: string }) => reference.url);
