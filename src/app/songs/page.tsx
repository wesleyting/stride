...
export default async function SongsPage({ searchParams }: { searchParams: Promise<{ added?: string }> }) {
  ...
  return <AppFrame showSidebar sidebarFooter={<SessionSidebarFooter signedIn isGuest={isGuest} next="/songs" />}><main className="min-w-0 flex-1 px-4 py-6 sm:px-7 sm:py-8"><header className="flex items-start justify-between gap-4"><h1 className="text-2xl font-semibold tracking-tight text-stone-950">All songs</h1><CreateItemModal activitySlug="guitar" activityKind="practice" isGuest={isGuest} folders={foldersReady ? folders : undefined} defaultSongPublic={preferencesResult.data?.default_song_public ?? false} defaultTuning={preferencesResult.data?.default_tuning ?? "standard"} /></header><SongLibrary key={libraryKey} songs={songs} folders={folders} foldersReady={foldersReady} referencesBySong={referencesBySong} addedSongSlug={added} /></main></AppFrame>;
}
