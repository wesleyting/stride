...
function PracticeLogEntry({ entry, itemSlug, suggestions }: { entry: EntryRecord; itemSlug: string; suggestions: string[] }) {
  ...
  return <article className="group/log px-4 py-4 sm:px-5"><div className="flex items-center justify-between gap-4"><div className="flex min-w-0 items-baseline gap-3"><p className="shrink-0 text-sm font-semibold text-stone-900"><LocalDateTime value={entry.created_at} display="time" /></p><p className="truncate text-sm text-stone-500">{entry.duration_seconds ? `${formatTrackedTime(entry.duration_seconds)} practice` : "Manual log"}</p></div><EditPracticeEntryModal entryId={entry.id} itemSlug={itemSlug} note={entry.content} rating={entry.rating} practicePart={entry.practice_part ?? ""} suggestions={suggestions} /></div>{hasDetails ? <details className="group/details mt-3 rounded-lg bg-stone-50"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 rounded-lg px-3 py-2 text-xs font-semibold text-stone-600 hover:bg-stone-100 focus-visible:ring-2 focus-visible:ring-stone-500"><span>View details</span><ChevronDown className="size-3.5 text-stone-400 transition-transform group-open/details:rotate-180" aria-hidden="true" /></summary><div className="grid gap-3 border-t border-stone-200 px-3 py-3">{tags.length ? <div><p className="text-[0.6875rem] font-semibold tracking-wide text-stone-400 uppercase">Worked on</p><div className="mt-1.5 flex flex-wrap gap-1">{tags.map((tag) => <span key={tag} className="rounded-md bg-white px-2 py-0.5 text-xs font-medium text-stone-600 ring-1 ring-stone-200">{tag}</span>)}</div></div> : null}{note ? <div><p className="text-[0.6875rem] font-semibold tracking-wide text-stone-400 uppercase">Note</p><p className="mt-1 text-sm leading-6 text-stone-700">{note}</p></div> : null}{entry.rating ? <div><p className="text-[0.6875rem] font-semibold tracking-wide text-stone-400 uppercase">Session rating</p><p className="mt-1 text-sm font-semibold tabular-nums text-stone-900">{entry.rating} / 10</p></div> : null}</div></details> : null}</article>;
}
